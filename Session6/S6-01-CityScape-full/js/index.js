...
function setupWorld() {
  ...
  var mat = new THREE.MeshNormalMaterial({color: 0x9db3b5, overdraw: true});
  ...
  var material = new THREE.MeshPhongMaterial({overdraw: true, color:Math.random() * 0xcccccc});
  ...
  for (var i = 0; i < 300; i++) {
    ...
    building.position.x = Math.floor( Math.random() * 300 - 150 ) * 4;
    building.position.z = Math.floor( Math.random() * 300 - 150 ) * 4;
    building.scale.x  = Math.pow(Math.random(), 2) * 50 + 30;
